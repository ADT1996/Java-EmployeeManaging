...
public class EmployeeJpaController implements Serializable {
    ...
    public void create(Employee employee) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            City nativeLand = employee.getNativeLand();
            if (nativeLand != null) {
                nativeLand = em.getReference(nativeLand.getClass(), nativeLand.getIdcity());
                employee.setNativeLand(nativeLand);
            }
            City takenCodePlace = employee.getTakenCodePlace();
            if (takenCodePlace != null) {
                takenCodePlace = em.getReference(takenCodePlace.getClass(), takenCodePlace.getIdcity());
                employee.setTakenCodePlace(takenCodePlace);
            }
            City takenLaborPlace = employee.getTakenLaborPlace();
            if (takenLaborPlace != null) {
                takenLaborPlace = em.getReference(takenLaborPlace.getClass(), takenLaborPlace.getIdcity());
                employee.setTakenLaborPlace(takenLaborPlace);
            }
            ...
            em.persist(employee);
            if (nativeLand != null) {
                nativeLand.getEmployeeCollection().add(employee);
                nativeLand = em.merge(nativeLand);
            }
            if (takenCodePlace != null) {
                takenCodePlace.getEmployeeCollection().add(employee);
                takenCodePlace = em.merge(takenCodePlace);
            }
            if (takenLaborPlace != null) {
                takenLaborPlace.getEmployeeCollection().add(employee);
                takenLaborPlace = em.merge(takenLaborPlace);
            }
            if (learning != null) {
    ...
    public void edit(Employee employee) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            City nativeLandOld = persistentEmployee.getNativeLand();
            City nativeLandNew = employee.getNativeLand();
            City takenCodePlaceOld = persistentEmployee.getTakenCodePlace();
            City takenCodePlaceNew = employee.getTakenCodePlace();
            City takenLaborPlaceOld = persistentEmployee.getTakenLaborPlace();
            City takenLaborPlaceNew = employee.getTakenLaborPlace();
            ...
            if (nativeLandNew != null) {
                nativeLandNew = em.getReference(nativeLandNew.getClass(), nativeLandNew.getIdcity());
                employee.setNativeLand(nativeLandNew);
            }
            if (takenCodePlaceNew != null) {
                takenCodePlaceNew = em.getReference(takenCodePlaceNew.getClass(), takenCodePlaceNew.getIdcity());
                employee.setTakenCodePlace(takenCodePlaceNew);
            }
            if (takenLaborPlaceNew != null) {
                takenLaborPlaceNew = em.getReference(takenLaborPlaceNew.getClass(), takenLaborPlaceNew.getIdcity());
                employee.setTakenLaborPlace(takenLaborPlaceNew);
            }
            if (learningNew != null) {
            ...
            if (nativeLandOld != null && !nativeLandOld.equals(nativeLandNew)) {
                nativeLandOld.getEmployeeCollection().remove(employee);
                nativeLandOld = em.merge(nativeLandOld);
            }
            if (nativeLandNew != null && !nativeLandNew.equals(nativeLandOld)) {
                nativeLandNew.getEmployeeCollection().add(employee);
                nativeLandNew = em.merge(nativeLandNew);
            }
            if (takenCodePlaceOld != null && !takenCodePlaceOld.equals(takenCodePlaceNew)) {
                takenCodePlaceOld.getEmployeeCollection().remove(employee);
                takenCodePlaceOld = em.merge(takenCodePlaceOld);
            }
            if (takenCodePlaceNew != null && !takenCodePlaceNew.equals(takenCodePlaceOld)) {
                takenCodePlaceNew.getEmployeeCollection().add(employee);
                takenCodePlaceNew = em.merge(takenCodePlaceNew);
            }
            if (takenLaborPlaceOld != null && !takenLaborPlaceOld.equals(takenLaborPlaceNew)) {
                takenLaborPlaceOld.getEmployeeCollection().remove(employee);
                takenLaborPlaceOld = em.merge(takenLaborPlaceOld);
            }
            if (takenLaborPlaceNew != null && !takenLaborPlaceNew.equals(takenLaborPlaceOld)) {
                takenLaborPlaceNew.getEmployeeCollection().add(employee);
                takenLaborPlaceNew = em.merge(takenLaborPlaceNew);
            }
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            City nativeLand = employee.getNativeLand();
            if (nativeLand != null) {
                nativeLand.getEmployeeCollection().remove(employee);
                nativeLand = em.merge(nativeLand);
            }
            City takenCodePlace = employee.getTakenCodePlace();
            if (takenCodePlace != null) {
                takenCodePlace.getEmployeeCollection().remove(employee);
                takenCodePlace = em.merge(takenCodePlace);
            }
            City takenLaborPlace = employee.getTakenLaborPlace();
            if (takenLaborPlace != null) {
                takenLaborPlace.getEmployeeCollection().remove(employee);
                takenLaborPlace = em.merge(takenLaborPlace);
            }
            Learning learning = employee.getLearning();
            if (learning != null) {
